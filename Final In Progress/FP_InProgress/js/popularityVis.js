...
class PopularityVis {
    ...
    initVis() {
        ...
        let heightReduction = 50;
        vis.height = document.getElementById(vis.parentElement).getBoundingClientRect().height - vis.margin.top - vis.margin.bottom - heightReduction;
    ...
    updateVis() {
        ...
        if(vis.svg.selectAll(".y-axis-label").empty()){
            vis.svg.append("text")
        ...
        // Append X-axis label if it does not already exist
        if(vis.svg.selectAll(".x-axis-label").empty()){
            vis.svg.append("text")
                .attr("class", "x-axis-label")
                .attr("x", vis.width)
                .attr("y", vis.height - 6) // Adjust this value to position the label correctly
